...
function LoginScreen() {
  ...
  useEffect(() => {
    if (isConnected) {
      const alreadyReloaded = sessionStorage.getItem('reloadedAfterConnect')
      if (alreadyReloaded) {
        navigate('/welcome', { replace: true })
      } else {
        sessionStorage.setItem('reloadedAfterConnect', '1')
        window.location.replace('/welcome')
      }
    } else {
      sessionStorage.removeItem('reloadedAfterConnect')
    }
